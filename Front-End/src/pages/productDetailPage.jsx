...
export default function ProductDetail({ setPage, listingId }) {
	...
	const loadListing = useCallback(async () => {
		...
		try {
			...
			setError("We couldn't load this listing. Check your connection and try again.");
		} finally {
			setLoading(false);
		}
	}, [listingId]);
	...
	if (!imageUrl || failed) {
		return (
			<div className="flex h-96 w-full items-center justify-center rounded-xl bg-sky-100">
				<ImageIcon className="size-20 text-sky-300 opacity-50" strokeWidth={1} />
			</div>
		);
	}
